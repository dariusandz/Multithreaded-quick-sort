    ...
    public static void main(String[] args) {
        if (args.length > 4) {
            System.out.println("" +
                ...
                " arg[2] - data sample value variation (1...n) \n" +
                " arg[3] - silent mode (false, true)");
        }

        int nThreads = Integer.parseInt(args[0]);
        int bound = Integer.parseInt(args[1]);
        int valueBound = Integer.parseInt(args[2]);

        // is setting Logger to log loudly with thread sleeps or only end results
        boolean silent = args[3] == null || Boolean.parseBoolean(args[3]);
        Logger.setSilent(silent);

        threadsAvailable = nThreads;

        final AtomicInteger activeThreadCount = new AtomicInteger(1);
        Integer[] arr = generateData(r -> r.nextInt(valueBound), Integer.class, bound);

        // execute quick sort
        Thread thread = new Thread(new Quicksorter<>(arr, 0, arr.length - 1, activeThreadCount, 1));
        thread.start();
        ...
        if (silent) {
            ...
            System.out.println("Data variation: 0..." + valueBound);
