...
public class Quicksorter<T extends Comparable<T>> implements Runnable {
    ...
    private void sort(int nLeft, int nRight) throws InterruptedException {
        if (nLeft < nRight) {
            ...
            Logger.log(nthThread + " thread says: " + activeThreadCount.get() + "/" + Main.threadsAvailable + " active threads. Should I create more? " + !shouldNotPartitionIntoThreads());
            if (shouldNotPartitionIntoThreads()) {
                ...
                Thread leftExecutor = new Thread(new Quicksorter<>(data, nLeft, partitionIndex - 1, activeThreadCount, activeThreadCount.get() - 2 + 1));
                leftExecutor.start();
                Thread rightExecutor = new Thread(new Quicksorter<>(data, partitionIndex + 1, nRight, activeThreadCount, activeThreadCount.get() - 2 + 2));
                rightExecutor.start();
...
//        synchronized (activeThreadCount) {
        return activeThreadCount.get() + 2 > Main.threadsAvailable;
//        }
    }
